...
void orbit()
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        {
            glRotatef(45, 0.2, 0.0, 0.0);
        }
        else {
            glRotatef(63, 1.0, 0.0, 0.0);
        }
        glScalef(sc[i] * 0.75, sc[i] * 0.75, sc[i] * 0.75);
...
void draw(void)
{
    ...
    for (j = 0; j < 4; j++)
    {
        glPushMatrix(); siz -= 0.5;
        glPointSize(siz);
        glScalef(scl[j], scl[j], scl[j]);
        glBegin(GL_POINTS);
        double ang1 = 0.0 - angleAstroid * (j/2 + 0.5), a = (2 * PI) / div ;
        for (i = 0; i < div; i++)
        {
            glVertex2d(cos(ang1), sin(ang1));
            ang1 += a;
    ...
    glPushMatrix();
    glRotatef(angleNeptune, 0.0, 0.5, -0.5);
